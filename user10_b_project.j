...
costsheet.Columns["B"].rows[rowid].value:="@%DB_ACC_CODE%";
;costsheet.Columns["C"].rows[rowid].select();
costsheet.Columns["C"].rows[rowid].value:="@%DB_ACC_SUPPLIER%";
...
fn := fn0 + fn1 + " MODELS TESTING(图)" + '_' + inttostr(slot_user10) +'_' + acc + '.bmp';
