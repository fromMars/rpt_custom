...
if a_link<>"" || a_linked.indexof("@%DB_COST_ARTICLE%")<>-1 then
{
    ...
    currentcell.formulaR1C1:="=(1+Data!AWRate)*@%DB_PIECE_CFLENGTH%/%ASSEMBLYCOUNT%+"+curr_profile_value;
	currentcell.borders.linestyle:=1;
}
else
{
...
else
{
	...
	{
		tot_formula:="="+RId+CId+Lbr+"1"+RBr+"/((1+Data!AWRate)*@%DB_PIECE_CFLENGTH%/%ASSEMBLYCOUNT%)";
		currentcell.formulaR1C1:=tot_formula;
	}
}
if @%DB_COST_ARTICLE%=16 || @%DB_COST_ARTICLE%=17 then
